...
class CustomSupplierQuote(models.Model):

    def _default_request_to_ids(self):
        group = self.env.ref('material_requisition.group_custom_supplier_quote_approver', raise_if_not_found=False)
        if group:
            return group.user_ids.ids
        return []
    _name = 'custom.supplier.quote'
    _description = 'Supplier Quote'
    ...
    line_ids = fields.One2many('custom.supplier.quote.line', 'document_id', string='Products', copy=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('custom.supplier.quote') or _('New')
...
class CustomSupplierQuoteLine(models.Model):
    _name = 'custom.supplier.quote.line'
    _description = 'Supplier Quote Line'

    document_id = fields.Many2one('custom.supplier.quote', string='Document Reference', required=True, ondelete='cascade', index=True, copy=False)
